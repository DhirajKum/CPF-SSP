...
public class OTPServiceImpl implements OTPService {
	...
	/*
	 * private static String USERNAME = "fcilekha.sms"; private static String PIN =
	 * "W3%24cV1%26tE5"; private static String SIGNATURE = "FCIFAP"; private static
	 * String DLT_ENTITY_ID="1101611580000043694"; private static String
	 * DLT_TEMPLATE_ID="1107161233369894498"; private static String DEL_BY_SMSPORTAL
	 * = "N";
	 */
	...
	public boolean sendOtpSms(String mobileNumber, String empNum, String empName) {
		...
		if (cpfOtpMaster != null && StringUtils.isNull(cpfOtpMaster.getOTP())) {
			...
			otpMessage = otpMessage.replace("<name>", empName.split(" ")[0].trim());
			String encodedMessage = otpMessage.replace(" ", "%20");
			empDao.addOtpData(mobileNumber, otp, otpExpiryTime, empNum, CPFCommonConstants.USERNAME,
					CPFCommonConstants.PIN, encodedMessage, CPFCommonConstants.SIGNATURE,
					CPFCommonConstants.DEL_BY_SMSPORTAL, CPFCommonConstants.DLT_ENTITY_ID,
					CPFCommonConstants.DLT_TEMPLATE_ID);
	...
	public String getValidatedNumber(String mobileNumber) {
		// Guard Clause
		if (StringUtils.isNull(mobileNumber))
			return null;
		...
		// No need to handle other numbers - as of now we are only handling India
		// numbers
		return null;
	}
	...
	public boolean verifyOtp(String mobile, String otp) {
		...
		try {
			if (cpfOtpMaster != null && StringUtils.isNotNull(cpfOtpMaster.getOTP())
					&& cpfOtpMaster.getOTP().equals(otp)) {
